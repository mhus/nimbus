...
@Slf4j
public class GetAssetsCommand implements WebSocketCommand {

    private final TerrainServiceClient terrainServiceClient;

    @Override
    public WebSocketCommandInfo info() {
        return new WebSocketCommandInfo("terrain", "getAssets", "Get paginated list of assets");
    }
    ...
    public ExecuteResponse execute(ExecuteRequest request) {
        try {
            ...
            Page<AssetDto> result = terrainServiceClient.getAssets(world, page, size);
            ...
            log.error("Error getting assets", e);
            return ExecuteResponse.error("GET_ASSETS_ERROR", "Failed to get assets: " + e.getMessage());
        }
    }
}
