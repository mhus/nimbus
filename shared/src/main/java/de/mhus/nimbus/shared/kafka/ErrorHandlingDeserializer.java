...
public class ErrorHandlingDeserializer<T> implements Deserializer<T> {
    ...
    public void configure(Map<String, ?> configs, boolean isKey) {
        ...
        try {
            ...
            log.info("Configured ErrorHandlingDeserializer with delegate: {} for {}",
                delegateClass, isKey ? "key" : "value");

        } catch (Exception e) {
            log.error("Failed to configure ErrorHandlingDeserializer: {}", e.getMessage(), e);
    ...
    public T deserialize(String topic, byte[] data) {
        try {
            if (data == null) {
                log.debug("Received null data for topic: {}", topic);
                return null;
            }

            T result = delegate.deserialize(topic, data);
            log.trace("Successfully deserialized {} message from topic: {}",
                isForKey ? "key" : "value", topic);
            return result;

        } catch (Exception e) {
            log.error("Failed to deserialize {} from topic '{}': {} - Data length: {} bytes",
            ...
            if (data != null && log.isDebugEnabled()) {
                ...
                log.debug("Raw data preview: {}", dataPreview);
            }
    ...
    public void close() {
        if (delegate != null) {
            try {
                delegate.close();
                log.debug("Closed delegate deserializer: {}", delegate.getClass().getSimpleName());
            } catch (Exception e) {
                log.warn("Error closing delegate deserializer: {}", e.getMessage(), e);
            }
        }
    }
