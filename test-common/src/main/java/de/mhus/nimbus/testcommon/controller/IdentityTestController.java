...
public class IdentityTestController {
    ...
    public ResponseEntity<String> requestLogin(
        ...
        try {
            log.info("REST: Requesting login for username: {}", username);
            ...
            log.info("Login response received for user '{}': status={}", username, response.getStatus());
            ...
            log.error("Failed to request login", e);
    ...
    public ResponseEntity<String> lookupUserById(@PathVariable Long userId) {
        try {
            log.info("REST: Looking up user by ID: {}", userId);
            ...
            log.info("User lookup response received for ID {}: status={}", userId, response.getStatus());
            ...
            log.error("User lookup request timed out for ID {}", userId);
            return ResponseEntity.status(408).body("User lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup user by ID", e);
    ...
    public ResponseEntity<String> lookupUserByUsername(@PathVariable String username) {
        try {
            log.info("REST: Looking up user by username: {}", username);
            CompletableFuture<UserLookupResponse> future = identityClient.lookupUserByUsername(username);

            // Wait for the response synchronously with timeout
            UserLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("User lookup response received for username '{}': status={}", username, response.getStatus());
            ...
            log.error("User lookup request timed out for username '{}'", username);
            return ResponseEntity.status(408).body("User lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup user by username", e);
            return ResponseEntity.internalServerError().body("Failed to lookup user: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> lookupUserByEmail(@PathVariable String email) {
        try {
            log.info("REST: Looking up user by email: {}", email);
            CompletableFuture<UserLookupResponse> future = identityClient.lookupUserByEmail(email);

            // Wait for the response synchronously with timeout
            UserLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("User lookup response received for email '{}': status={}", email, response.getStatus());
            ...
            log.error("User lookup request timed out for email '{}'", email);
            return ResponseEntity.status(408).body("User lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup user by email", e);
            return ResponseEntity.internalServerError().body("Failed to lookup user: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> lookupCharacterById(@PathVariable Long characterId) {
        try {
            log.info("REST: Looking up character by ID: {}", characterId);
            ...
            log.info("Character lookup response received for ID {}: status={}", characterId, response.getStatus());
            ...
            log.error("Character lookup request timed out for ID {}", characterId);
            return ResponseEntity.status(408).body("Character lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup character by ID", e);
    ...
    public ResponseEntity<String> lookupCharacterByName(@PathVariable String characterName) {
        try {
            log.info("REST: Looking up character by name: {}", characterName);
            CompletableFuture<PlayerCharacterLookupResponse> future = identityClient.lookupCharacterByName(characterName);

            // Wait for the response synchronously with timeout
            PlayerCharacterLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("Character lookup response received for name '{}': status={}", characterName, response.getStatus());
            ...
            log.error("Character lookup request timed out for name '{}'", characterName);
            return ResponseEntity.status(408).body("Character lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup character by name", e);
            return ResponseEntity.internalServerError().body("Failed to lookup character: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> lookupCharactersByUserId(@PathVariable Long userId) {
        try {
            log.info("REST: Looking up characters for user ID: {}", userId);
            CompletableFuture<PlayerCharacterLookupResponse> future = identityClient.lookupCharactersByUserId(userId);

            // Wait for the response synchronously with timeout
            PlayerCharacterLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("Characters lookup response received for user ID {}: status={}", userId, response.getStatus());
            ...
            log.error("Characters lookup request timed out for user ID {}", userId);
            return ResponseEntity.status(408).body("Characters lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup characters by user ID", e);
    ...
    public ResponseEntity<String> requestPublicKey() {
        try {
            log.info("REST: Requesting public key");
            ...
            log.info("Public key response received: status={}", response.getStatus());
            ...
            log.error("Failed to request public key", e);
    ...
    public ResponseEntity<String> createAce(
        ...
        try {
            log.info("REST: Creating ACE for user {} with rule: {}", userId, rule);
            ...
            log.info("ACE create response received: success={}", response.getSuccess());
    ...
    public ResponseEntity<String> createAceWithOrder(
            ...
            @RequestParam(required = false) String description) {
        try {
            log.info("REST: Creating ACE for user {} with rule: {} and order: {}", userId, rule, orderValue);
            CompletableFuture<AceCreateResponse> future = identityClient.createAceWithOrder(rule, userId, orderValue, description);

            // Wait for the response synchronously with timeout
            AceCreateResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("ACE create with order response received: success={}", response.getSuccess());
    ...
    public ResponseEntity<String> lookupAceById(@PathVariable Long aceId) {
        try {
            log.info("REST: Looking up ACE by ID: {}", aceId);
            ...
            log.info("ACE lookup response received: success={}, found {} ACEs", response.getSuccess(), response.getAces().size());

            return ResponseEntity.ok(formatAceLookupResponse(response, "ACE Lookup by ID"));

        } catch (java.util.concurrent.TimeoutException e) {
            log.error("ACE lookup request timed out for ID {}", aceId);
            return ResponseEntity.status(408).body("ACE lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup ACE by ID", e);
    ...
    public ResponseEntity<String> lookupAcesByUserId(
        ...
        try {
            log.info("REST: Looking up ACEs for user ID: {} (activeOnly: {})", userId, activeOnly);
            CompletableFuture<AceLookupResponse> future = identityClient.lookupAcesByUserId(userId, activeOnly);

            // Wait for the response synchronously with timeout
            AceLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("ACEs lookup response received: success={}, found {} ACEs", response.getSuccess(), response.getAces().size());

            return ResponseEntity.ok(formatAceLookupResponse(response, "ACEs Lookup by User ID"));

        } catch (java.util.concurrent.TimeoutException e) {
            log.error("ACEs lookup request timed out for user ID {}", userId);
            return ResponseEntity.status(408).body("ACEs lookup request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup ACEs by user ID", e);
    ...
    public ResponseEntity<String> lookupAcesByRule(
            ...
            @RequestParam(defaultValue = "false") boolean activeOnly) {
        try {
            log.info("REST: Looking up ACEs with rule pattern: {} (activeOnly: {})", rulePattern, activeOnly);
            CompletableFuture<AceLookupResponse> future = identityClient.lookupAcesByRule(rulePattern, activeOnly);

            // Wait for the response synchronously with timeout
            AceLookupResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("ACEs lookup by rule response received: success={}, found {} ACEs", response.getSuccess(), response.getAces().size());

            return ResponseEntity.ok(formatAceLookupResponse(response, "ACEs Lookup by Rule Pattern"));

        } catch (java.util.concurrent.TimeoutException e) {
            log.error("ACEs lookup by rule request timed out for pattern '{}'", rulePattern);
            return ResponseEntity.status(408).body("ACEs lookup by rule request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to lookup ACEs by rule", e);
    ...
    public ResponseEntity<String> updateAce(
        ...
        try {
            log.info("REST: Updating ACE with ID: {}", aceId);
            ...
            log.info("ACE update response received: success={}", response.getSuccess());
            ...
            log.error("ACE update request timed out for ID {}", aceId);
            return ResponseEntity.status(408).body("ACE update request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to update ACE", e);
    ...
    public ResponseEntity<String> deleteAce(@PathVariable Long aceId) {
        try {
            log.info("REST: Deleting ACE with ID: {}", aceId);
            ...
            log.info("ACE delete response received: success={}, deleted {} ACEs", response.getSuccess(), response.getDeletedCount());
            ...
            log.error("ACE delete request timed out for ID {}", aceId);
            return ResponseEntity.status(408).body("ACE delete request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to delete ACE", e);
    ...
    public ResponseEntity<String> deleteAllAcesForUser(@PathVariable Long userId) {
        try {
            log.info("REST: Deleting all ACEs for user ID: {}", userId);
            CompletableFuture<AceDeleteResponse> future = identityClient.deleteAllAcesForUser(userId);

            // Wait for the response synchronously with timeout
            AceDeleteResponse response = future.get(30, java.util.concurrent.TimeUnit.SECONDS);

            log.info("ACE delete all response received: success={}, deleted {} ACEs", response.getSuccess(), response.getDeletedCount());
            ...
            log.error("ACE delete all request timed out for user ID {}", userId);
            return ResponseEntity.status(408).body("ACE delete all request timed out after 30 seconds");
        } catch (Exception e) {
            log.error("Failed to delete all ACEs for user", e);
