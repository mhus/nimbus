...
public class DemoSetupApplication implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        var adminPasswordOpt = adminService.getAdminPassword();
        adminPasswordOpt.ifPresentOrElse(
        ...
        adminToken = adminPasswordOpt.flatMap(this::loginAdmin).orElse(null);
        if (adminToken == null) {
            log.warn("Admin Login fehlgeschlagen – /shared/key Aufrufe ohne Auth (falls erlaubt)");
            universeClient = initialClient; // Fallback
        } else {
    ...
    private void registerRegionServerPublicKey() {
        regionServerPublicKeyService.getRegionServerPublicKey().ifPresentOrElse(
                raw -> {
                    try {
                        ...
                        if (pubOpt.isEmpty()) {
                            ...
                            boolean exists = existsKeyInUniverse("REGION","PUBLIC", regionServerId, MAIN_JWT_TOKEN);
                            if (!exists) {
                                createKeyInUniverse("REGION","PUBLIC", pub.getAlgorithm(), regionServerId, MAIN_JWT_TOKEN, regionServerId + ":" + MAIN_JWT_TOKEN + ":" + java.util.UUID.randomUUID(), base64);
                            } else {
    ...
    private java.util.Optional<String> loginAdmin(String password) {
        try {
