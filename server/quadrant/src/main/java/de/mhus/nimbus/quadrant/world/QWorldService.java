...
@Service
public class QWorldService {

    public static class WorldAccess {
        private final QWorld world;
        private final java.util.Set<String> roles;
        public WorldAccess(QWorld world) { this.world = world; this.roles = new java.util.LinkedHashSet<>(); }
        public QWorld getWorld() { return world; }
        public java.util.Set<String> getRoles() { return roles; }
        public void addRole(String role) { if (role!=null) roles.add(role); }
    }

    private final QWorldRepository repository;

    public QWorldService(QWorldRepository repository) {
        this.repository = repository;
    }

    // CRUD
    public QWorld create(String worldId, String name) {
        ...
        QWorld w = new QWorld(worldId, name);
        return repository.save(w);
    }

    public Optional<QWorld> getById(String id) {
        return repository.findById(id);
    }

    public Optional<QWorld> getByWorldId(String worldId) {
        return repository.findByWorldId(worldId);
    }

    public List<QWorld> listAll() { return repository.findAll(); }
    ...
    public List<WorldAccess> getAccessForUser(String userId) {
        ...
        java.util.function.BiConsumer<List<QWorld>, String> addAll = (list, role) -> {
            for (QWorld w : list) {
    ...
    public QWorld update(String id,
                         String name,
                         String description,
                         String apiUrl,
                         String websocketUrl,
                         String controlsUrl,
                         QWorld.Visibility visibility,
                         String branch,
                         QWorld.State state,
                         String ownersCsv,
                         String editorsCsv,
                         ...
                         String playersCsv,
                         List<QWorld.EntryPoint> entryPoints
    ) {
        QWorld existing = repository.findById(id)
    ...
    // membership helpers
    public QWorld addOwner(String id, String userId) { return updateListMember(id, userId, ListType.OWNERS, true); }
    public QWorld removeOwner(String id, String userId) { return updateListMember(id, userId, ListType.OWNERS, false); }
    public QWorld addEditor(String id, String userId) { return updateListMember(id, userId, ListType.EDITORS, true); }
    public QWorld removeEditor(String id, String userId) { return updateListMember(id, userId, ListType.EDITORS, false); }
    public QWorld addMaintainer(String id, String userId) { return updateListMember(id, userId, ListType.MAINTAINERS, true); }
    public QWorld removeMaintainer(String id, String userId) { return updateListMember(id, userId, ListType.MAINTAINERS, false); }
    public QWorld addPlayer(String id, String userId) { return updateListMember(id, userId, ListType.PLAYERS, true); }
    public QWorld removePlayer(String id, String userId) { return updateListMember(id, userId, ListType.PLAYERS, false); }

    public QWorld upsertEntryPoint(String id, QWorld.EntryPoint ep) {
        QWorld existing = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("World not found: " + id));
        existing.upsertEntryPoint(ep);
        return repository.save(existing);
    }
    public QWorld removeEntryPoint(String id, String name) {
        QWorld existing = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("World not found: " + id));
    ...
    private QWorld updateListMember(String id, String userId, ListType type, boolean add) {
